...
import com.databases.erd.erdtool.RunQueries.RunPostgreQuery;
import com.databases.erd.erdtool.XmlToDb.xmltodb;

public class GeneratePostgreQuery {
	public static void Generate(String dbname)
	{
		...
		for(String table:tables)
		{
			...
			for(int i=0;i<columns.size();i++)
			{
				...
				if(types.get(i).equals("double")||types.get(i).equals("DOUBLE")||types.get(i).equals("datetime")||types.get(i).equals("DATETIME")||types.get(i).equals("INT")||types.get(i).equals("BIGINT")||types.get(i).equalsIgnoreCase("INT"))
				{
					qry=qry+ConvertToPostgre.convertToPostgreType(types.get(i))+" ";
				}
				else
				{
					qry=qry+ConvertToPostgre.convertToPostgreType(types.get(i))+"("+precisions.get(i)+") ";
				}
